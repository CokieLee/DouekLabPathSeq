...
single_demux_row=$(ls -l ../Demux_Samples_BCL_to_FQ_format | awk -v DemuxIndex=$DemuxIndex 'NR==DemuxIndex')
echo $single_demux_row

FileDescArray=($single_demux_row)
BCLFQ_read_file_name=${FileDescArray[8]}
echo $BCLFQ_read_file_name
IFS=$OLD_IFS

##Extract the components of the read file name from the BCLFQ naming convention
OLD_IFS=$IFS
IFS='_'
BCLFQ_format_file_name_array=($BCLFQ_read_file_name)
origin=${BCLFQ_format_file_name_array[0]}
well_plate=${BCLFQ_format_file_name_array[1]}
sample_index=${BCLFQ_format_file_name_array[2]}
lane=${BCLFQ_format_file_name_array[3]}
read=${BCLFQ_format_file_name_array[4]}
last_segment=${BCLFQ_format_file_name_array[5]}
IFS=$OLD_IFS
