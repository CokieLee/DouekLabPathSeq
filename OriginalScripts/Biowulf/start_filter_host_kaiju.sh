...
echo "Line 141"
echo $filterScript
filterJobID=$(sbatch --parsable --array=1-$fileCount%100 $filterScript $projectID $left_read_file_base_name $right_read_file_base_name $origin $program_PathSeqRemoveHostForKaiju $blastDB_Mammalia  | cut -d' ' -f3)
...
echo "line 151"
echo $kaijuScript
kaijuJobID=$(sbatch --dependency=afterok:$filterJobID --parsable $kaijuScript $projectID  $left_read_file_base_name $right_read_file_base_name $origin $fileCount $kaiju_nodes $kaiju_fmi | cut -d' ' -f3)
